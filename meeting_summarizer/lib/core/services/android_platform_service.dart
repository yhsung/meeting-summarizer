...
class AndroidPlatformService implements PlatformIntegrationInterface, PlatformSystemInterface {
  static const String _logTag = 'AndroidPlatformService';
  static const String _channelName = 'com.yhsung.meeting_summarizer/android_platform';
  ...
  // Callbacks
  void Function(String action, Map<String, dynamic>? parameters)? onPlatformAction;
  void Function(String assistantCommand, Map<String, dynamic>? parameters)? onAssistantCommand;
  ...
  Future<bool> initialize() async {
    ...
    try {
      if (!Platform.isAndroid) {
        log('$_logTag: Android platform services only available on Android', name: _logTag);
        return false;
      }
      ...
      _isInitialized = true;
      log('$_logTag: Android platform services initialized successfully', name: _logTag);
      return true;
      
    } catch (e) {
      log('$_logTag: Failed to initialize Android platform services: $e', name: _logTag);
      return false;
    }
  }
  ...
  Future<void> _initializePlatformChannel() async {
    try {
      ...
      if (!result) {
        log('$_logTag: Failed to initialize native Android platform channel', name: _logTag);
      }
  ...
  Future<void> _handleNativeMethodCall(MethodCall call) async {
    try {
      log('$_logTag: Received native method call: ${call.method}', name: _logTag);
      ...
      switch (call.method) {
        ...
        default:
          log('$_logTag: Unknown native method call: ${call.method}', name: _logTag);
      }
    } catch (e) {
  ...
  Future<void> updateAndroidAutoDisplay({
    ...
    try {
      await _androidAutoService.updateAutoDisplay(
        title: isRecording ? 'Meeting Recorder - Recording' : 'Meeting Recorder - Ready',
  ...
  Future<void> _initializeQuickSettingsTile() async {
    try {
      final result = await _platform.invokeMethod<bool>('setupQuickSettingsTile') ?? false;
      _quickSettingsTileEnabled = result;
      
      if (result) {
        log('$_logTag: Quick Settings tile enabled successfully', name: _logTag);
      } else {
        log('$_logTag: Failed to enable Quick Settings tile', name: _logTag);
      }
    } catch (e) {
      log('$_logTag: Error initializing Quick Settings tile: $e', name: _logTag);
    }
  }
  ...
  Future<void> _handleQuickSettingsTileClick() async {
    try {
      ...
      log('$_logTag: Error handling Quick Settings tile click: $e', name: _logTag);
    }
  }
  ...
  Future<void> updateQuickSettingsTile({
    ...
    try {
      await _platform.invokeMethod('updateQuickSettingsTile', {
        'isRecording': isRecording,
        'status': status ?? (isRecording ? 'Recording Active' : 'Tap to Record'),
  ...
  Future<void> _initializeHomeScreenWidgets() async {
    try {
      ...
      log('$_logTag: Error initializing home screen widgets: $e', name: _logTag);
    }
  }
  ...
  Future<void> _setupAdvancedWidgetLayouts() async {
    try {
      ...
      log('$_logTag: Failed to setup advanced widget layouts: $e', name: _logTag);
    }
  }
  ...
  Future<void> updateHomeScreenWidgets({
    ...
    try {
      ...
      await HomeWidget.saveWidgetData('last_sync', DateTime.now().toIso8601String());
  ...
  String _getWidgetStatus(bool isRecording, bool isTranscribing, bool lowPowerMode) {
  ...
  Future<void> _initializeGoogleAssistant() async {
    try {
      final result = await _platform.invokeMethod<bool>('setupGoogleAssistant') ?? false;
      
      if (result) {
        log('$_logTag: Google Assistant integration enabled', name: _logTag);
        await _registerAssistantActions();
      } else {
        log('$_logTag: Google Assistant integration not available', name: _logTag);
      }
    } catch (e) {
      ...
      final actions = [
        {
          ...
          'phrases': ['start recording meeting', 'begin recording', 'record meeting'],
        ...
        },
        {
          ...
          'phrases': ['transcribe recording', 'convert to text', 'generate transcript'],
  ...
  Future<void> _handleAssistantCommand(String command, Map<String, dynamic>? parameters) async {
    try {
  ...
  Future<void> _provideAssistantFeedback(String command) async {
    try {
      ...
      await _platform.invokeMethod('speakAssistantFeedback', {'text': feedback});
  ...
  Future<void> _updateAssistantResult(String command, bool success, {String? error}) async {
    try {
  ...
  Future<void> _initializeWorkProfileSupport() async {
    try {
      final result = await _platform.invokeMethod<Map>('checkWorkProfileSupport');
  ...
  Future<void> _setupWorkProfileFeatures(Map workProfileInfo) async {
    try {
      ...
      final hasWorkPolicyRestrictions = workProfileInfo['hasPolicyRestrictions'] as bool? ?? false;
  ...
  Future<void> _configureWorkProfileSecurity() async {
    try {
      ...
      log('$_logTag: Failed to configure work profile security: $e', name: _logTag);
    }
  }
  ...
  Future<void> _setupWorkProfileCompliance() async {
    try {
      ...
      log('$_logTag: Failed to setup work profile compliance: $e', name: _logTag);
    }
  }
  ...
  Future<void> _applyWorkProfileRestrictions(Map restrictions) async {
    try {
      await _platform.invokeMethod('applyWorkProfileRestrictions', restrictions);
      log('$_logTag: Work profile restrictions applied', name: _logTag);
    } catch (e) {
      log('$_logTag: Failed to apply work profile restrictions: $e', name: _logTag);
    }
  }
  ...
  Future<void> _initializeForegroundService() async {
    try {
      final result = await _platform.invokeMethod<bool>('initializeForegroundService') ?? false;
  ...
  @override
  Future<void> handleAction(String action, Map<String, dynamic>? parameters) async {
    try {
  ...
  Future<void> updateIntegrations(Map<String, dynamic> state) async {
    try {
      ...
      final transcriptionProgress = state['transcriptionProgress'] as double? ?? 0.0;
      ...
      if (_foregroundServiceActive && isRecording) {
        await updateForegroundService(
          ...
          progress: isTranscribing ? (transcriptionProgress * 100).toInt() : null,
